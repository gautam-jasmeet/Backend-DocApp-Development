...
const queryDb = async (query, params = []) => {
  ...
  try {
    const [result] = await connection.query(query, params); // Execute the query
    return result; // Return the result
  } catch (err) {
    throw err; // Throw any errors for handling in the calling function
  } finally {
...
// Update Document Status by ID (Admin)
export const updateDocumentStatus = async (req, res) => {
  const query = "UPDATE documents SET status = ? WHERE id = ?";
  try {
    const result = await queryDb(query, [req.body.status, req.params.id]);
