...
export const updateJoiningForm = async (req, res) => {
  try {
    ...
    // File handling for photo upload
    if (req.file) {
      updatedFields.photo_url = `/uploads/${encodeURIComponent(
    ...
    const query = `UPDATE joining_forms SET ${fieldsToUpdate} WHERE id = ?`;
    const values = [...Object.values(updatedFields), id]; // Spread updatedFields values and append ID

    await db.promise().query(query, values);

    res.status(200).json({ message: "Joining form updated successfully" }); // 200 for success
  } catch (err) {
    res.status(500).json({ error: err.message }); // 500 for server error
  }
};
...
export const deleteJoiningForm = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Joining form deleted successfully" }); // 200 for success
  } catch (err) {
    res.status(500).json({ error: err.message }); // 500 for server error
  }
};
